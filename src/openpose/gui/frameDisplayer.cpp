...
namespace op
{
        ...
        mGuiDisplayMode{(fullScreen ? GuiDisplayMode::FullScreen : GuiDisplayMode::Windowed)}
    {
        try
        {
            ...
            if (mWindowedSize.x <= 0 || mWindowedSize.y <= 0)
                mWindowedSize = Point<int>{1280, 720};
        }
	...
	void FrameDisplayer::imrotate(const cv::Mat& matImage, cv::Mat& matNewImage, double dAngle){
		cv::Point2f pt2f(matImage.cols/2, matImage.rows/2);
		cv::Mat matrotate = cv::getRotationMatrix2D(pt2f, dAngle, 1.0);
		cv::warpAffine(matImage, matNewImage, matrotate, cv::Size(matNewImage.rows, matNewImage.cols));
	}
    ...
    void FrameDisplayer::displayFrame(const cv::Mat& frame, const int waitKeyValue)
    {
        try
        {
			...
			//cv::Mat matdisplayframe;
			//imrotate(frame, matdisplayframe, 90.0);
