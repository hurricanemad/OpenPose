...
namespace op
{
	...
	void VideoCaptureReader::imrotate(const cv::Mat& matImage, cv::Mat& matNewImage, double dAngle){
		cv::Point2f pt2f(matImage.cols/2, matImage.rows/2);
		cv::Mat matrotate = cv::getRotationMatrix2D(pt2f, dAngle, 1.0);
		int nmatWidth = matImage.cols;
		int nmatHeight = matImage.rows;
        
        cv::warpAffine(matImage, matNewImage, matrotate, cv::Size(nmatWidth, nmatWidth));
		cv::Rect rect((nmatWidth - nmatHeight)/2, 0, nmatHeight, nmatWidth);
		matNewImage = matNewImage(rect);

		//cv::imshow("matNewImage", matNewImage);
		//cv::waitKey(-1);
	}
    ...
    cv::Mat VideoCaptureReader::getRawFrame()
    {
        try
        {
			...
			//imrotate(frame, matCameraFrame, 90.0);
			//std::cout << "matCameraFrame cols:" << matCameraFrame.cols << std::endl;
			//std::cout << "matCameraFrame rows:" << matCameraFrame.rows << std::endl;
            return frame;
        }
        catch (const std::exception& e)
        {
